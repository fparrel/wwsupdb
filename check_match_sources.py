...
def main():
    ...
    names_evo=[]
    names_rivermap=[]
    cptrs = {'osm':0,'evo':0,'rivermap':0,'merged':0}
    # get merged rivers
    ...
    names.sort()
    print '<!DOCTYPE html>'
    print '<html><head><style>table, th, td {border: 1px solid black;}</style><meta http-equiv="Content-Type" content="text/html; charset=utf8" /></head><body><b>Merged: %s</b><table><tr><th>cleaned name %s</th><th>evo %s</th><th>rivermap %s</th><th>osm %s</th></tr>' % (cptrs['merged'],len(names),cptrs['evo'],cptrs['rivermap'],cptrs['osm'])
    for name in names:
        sources = rivers[name]
        print '<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>'% (name.encode('utf8'),none_or_encode(sources.get('name_evo')),none_or_encode(sources.get('name_rivermap')),none_or_encode(sources.get('name_osm')))
